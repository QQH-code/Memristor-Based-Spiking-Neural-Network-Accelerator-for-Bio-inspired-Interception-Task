...
% ----- output file -----
scs_file = sprintf('plot_integration1_%s.scs', task_id_str);
...
% Initial condition
%% =========================
fprintf(fid, 'ic N1.net6=0.92749 N1.net10=0 out=0.9 Tout=0.9 in_mid=0.9 in_node=0.9 gate_node=0\n\n');
...
fprintf(fid, ' V(vin_src) V(in_mid) V(in_node) V(gate_node) V(out) V(Tout) V(N1.net6) V(N1.net10)');
fprintf(fid, '\n');
fclose(fid);
...
if isempty(idx_vin_src) || isempty(idx_net6) || isempty(idx_out)
...
subplot(2,1,2);
plot(time_ns, v_net6, 'Color', c_blue, 'LineWidth', 1.5);
hold on;
